...
SELECT
  VARIANCE(replacement_cost) AS variabilidad_coste_reemplazo
FROM film;
...
-- Cálculo del promedio sin redondear
SELECT
  AVG(rental_duration)
FROM film;
...
-- 31. Obtener todas las películas y mostrar los actores que han actuado enellas, incluso si algunas películas no tienen actores asociados.
...
-- CONCLUSIÓN: esta consulta combina cada película con cada una de las categorías que existen.
-- Esto no ocurre en la vida real, pues una película no pertenece a todas las categorías existentes.
-- Por tanto esta consulta no aporta ningún valor analítico.
...
-- Para corroborar que funciona la vista creada usamos la siguiente consulta:
select *
...
-- Para corroborar que funciona la tabla temporal creada usamos la siguiente consulta:
select *
...
-- Queremos comprobar cuándo se alquiló por primera vez Spartacus Cheaper y usamos la siguiente consulta:

SELECT MIN(rental.rental_date) AS primera_vez_alquilada
FROM rental
JOIN inventory
  ON rental.inventory_id = inventory.inventory_id
JOIN film
...
-- Queremos comprobar que las fechas de alquiler que mostramos en nuestra consulta son correctas (posteriores a la primera vez que se alquiló Spartacus Cheaper) y usamos la siguiente consulta:
